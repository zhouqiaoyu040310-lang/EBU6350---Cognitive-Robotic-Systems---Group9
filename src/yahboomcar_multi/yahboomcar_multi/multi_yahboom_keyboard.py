	...
	try:
		...
		while (1):
			...
			if key in moveBindings.keys():
				...
				if (key == '\x03'): break
			if xspeed_switch: 
				robot1_twist.linear.x = speed * x
				robot2_twist.linear.x = speed * x
			else: 
				robot1_twist.linear.y = speed * x
				robot2_twist.linear.y = speed * x
			robot1_twist.angular.z = turn * th
			robot2_twist.angular.z = turn * th
			if not stop: 
				yahboom_keyboard.pub_robot1.publish(robot1_twist)
				yahboom_keyboard.pub_robot2.publish(robot2_twist)
			if stop:
				yahboom_keyboard.pub_robot1.publish(Twist())
				yahboom_keyboard.pub_robot2.publish(Twist())
